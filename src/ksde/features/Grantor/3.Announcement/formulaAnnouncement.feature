@formulaAnnouncements @grantor-regression @grantor-parallel-regression @announcement @regression @AnnouncementTemp1
Feature: Validate all scenarios in the Formula Announcement tab
  ...
  Scenario: Validate that user can not enter application due date as past date | Validate the submission of Announcement for Approval || Validate that only record owner cannot delete Submitted for Approval status announcements
    ...
    And I wait for "2" seconds
    Then I softly see value "$1,000" for title "Allocated Amount" against the value "SAN DIEGO ELECTRICAL TRAINING TRUST" inside table "---announcement:-:formulaInvitedApplicatsTableId---"
  ...
  @announcementApprovalFA @announcementPendingTaskPostApprovalFA @announcementCompletedTaskPostApprovalFA @Bug-Ticket-530694
  ...
  @announcementPostRejectionCheckInListViewFA @announcementPostRejectCompletedTaskForUserFA @announcementApprovalRejectionFA @announcementRejectPendingTaskForNextApproverFA @announcementPostRejectPendingTaskForUserFA @announcementPostRejectCheckFieldHistoryFA @FieldHistoryExclude @Bug-Ticket-530694
