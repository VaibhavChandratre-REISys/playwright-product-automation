@announcement @grantor-regression @grantor-parallel-regression @regression @annWithNotes @annExecution
Feature: Validate all notes scenarios in the  Announcement tab
  ...
  Scenario:Validate PM/ADMIN user cant add notes on directed announcement which is Closed
    ...
    When I navigate to "Directed" content inside "Announcements" subheader on left panel
    And I click toggle button to select "All Directed Announcements"
    And I perform quick search for "{SavedValue:ClosedStateAnnouncement_ApplicationSubmittedToGrantor}" in "---announcement:-:annoucementTableId---" panel
    And I click on "View" icon for "{SavedValue:ClosedStateAnnouncement_ApplicationSubmittedToGrantor}" inside flex table with id "---announcement:-:annoucementTableId---" without waiting for record
    And I navigate to "Files" sub tab
    Then I softly cannot see top right button "New" in flex table with id "---announcement:-:AnnouncementNotestableId---"
    When I re-login to "Grantor" app as "ADMIN" user on "INTERNAL" portal
    And I navigate to "Announcements" tab
  ...
  Scenario:Validate FD/PO user cant add notes on formula announcement which is Closed
    ...
    When I navigate to "Formula" content inside "Announcements" subheader on left panel
    And I click toggle button to select "All Formula Announcements"
    And I perform quick search for "---AUTOEnvData:-:Automation_Formula_Closed_Announcement_By_Applicant---" in "---announcement:-:formulaAnnouncementTableId---" panel
    And I click on "View" icon for "---AUTOEnvData:-:Automation_Formula_Closed_Announcement_By_Applicant---" inside flex table with id "---announcement:-:formulaAnnouncementTableId---" without waiting for record
    And I navigate to "Files" sub tab
    Then I softly cannot see top right button "New" in flex table with id "---announcement:-:AnnouncementNotestableId---"
    When I re-login to "Grantor" app as "PO" user on "INTERNAL" portal
    And I navigate to "Announcements" tab
