...
Feature: Validate all scenarios in Closeout
  ...
  Scenario Outline: Validate  EXE user see 'Balance' value for balance column which is Subtraction of Cumulative Obligation and Spent
    ...
    And I click on "Submit to Grantor" in the page details
    Then I softly see field "Status" as "Submitted to Grantor"
    When I re-login to "Grantor" app as "<Profile>" user on "INTERNAL" portal
    And I navigate to "Monitoring" tab
    And I navigate to "Pending Tasks" content inside "My Tasks" subheader on left panel
    When I perform quick search for "{SavedValue:PayreqId}" in "---paymentRequest:-:pendingTaskTableId---" panel
    And I click on "Start" icon for "{SavedValue:PayreqId}" inside flex table with id "---paymentRequest:-:pendingTaskTableId---" without waiting for record
  ...
  @247872 @ValidateEXEusershouldseeBudgetSummaryByFocusAreaSectionOnlyWhenGrantStatusIsClose @Allocation
  Scenario: Verify EXE user should see "Budget summary by focus area" section only when Grant status is close
    # Setup: activate a subaward with TWO focus areas (Focus Area = Yes) owned by EXE user
    Given I activated standalone subaward "Automation Runtime Award for closeout" with properties "SECOND_FOCUS_AREA-Automation EXE user" of type "Competitive"
    When I login to "Grantor" app as "EXE" user
    And I navigate to "Grants" tab
    When I navigate to "Grants" content inside "Grants, Subawards & Amendments" subheader on left panel
    And I click toggle button to select "Grants - Active"
    ...
    And I click on "View" icon for "{SavedValue:Automation Runtime Award for closeout}" inside flex table with id "---closeout:-:grantsTableId---"
    Then I softly see field "Status" as "Active"
    # Create an ad-hoc closeout request
    When I navigate to "Requests" sub tab
    And I click on top right button "New" in flex table with id "---closeout:-:closeoutTableId---"
    When I enter in modal "Creation" values from "Closeout_Field_Values.xlsx"
    And I click modal button "Save and Continue"
    ...
    When I enter the following values into flex table with id "---closeout:-:closeoutApproverTableId---" by clicking "New" :
      ...
      | {SavedValue:PM Username} | Step 1   |
    And I click on "Send To Subrecipient" in the page details
    # Subrecipient enters closeout fields and acknowledges
    When I re-login to "Grants Portal" app as "SPI3" user on "SUBPORTAL" portal
    When I navigate to "Closeout" tab
    And I navigate to "Pending Tasks" content inside "My Tasks" subheader on left panel
    When I perform quick search for "{SavedValue:closeoutID}" in "---closeout:-:pendingTaskTableId---" panel
    ...
    When I click on "Save" in the page details
    And I click on "Acknowledge" in the page details
    # Grant owner (EXE) overrides and submits for approval
    When I re-login to "Grantor" app as "EXE" user on "INTERNAL" portal
    When I navigate to "Closeout" tab
    And I navigate to "Pending Tasks" content inside "My Tasks" subheader on left panel
    And I perform quick search for "{SavedValue:closeoutID}" in "---closeout:-:pendingTaskTableId---" panel
    ...
    When I click on "Submit for Approval" in the page details
    And I wait for "2" seconds
    # Approver (PM) approves the closeout, moving the grant to Closed/Completed
    When I re-login to "Grantor" app as "PM" user on "INTERNAL" portal
    When I navigate to "Closeout" tab
    And I navigate to "Pending Tasks" content inside "My Tasks" subheader on left panel
    And I perform quick search for "{SavedValue:closeoutID}" in "---closeout:-:pendingTaskTableId---" panel
    ...
    When I "Approve" in the approval decision
    And I wait for "4" seconds
    # Verify as EXE: grant is Closed/Completed and "Budget summary by focus area" is visible on the Budget tab
    When I re-login to "Grantor" app as "EXE" user on "INTERNAL" portal
    And I navigate to "Grants" tab
    When I navigate to "Grants" content inside "Grants, Subawards & Amendments" subheader on left panel
    And I click toggle button to select "Grants - All"
  ...
  @247777 @VerifyEXEUserShouldSeeCumulativeObligationAndTotalCommitmentOnClosedGrant
  Scenario: Verify EXE user should see cumulative Obligation and total commitment field amount on grant if grant is in close state
    # ── Prerequisites: Create formula grant with focus area and activate award ──
    Given I activated standalone subaward "Automation Runtime Formula Grant for Closeout" with properties "SECOND_FOCUS_AREA_BUDGET_CATEGORY" of type "Formula_By_Applicant_Focus_Area"
    When I login to "Grantor" app as "EXE" user
    And I navigate to "Grants" tab
    When I navigate to "Grants" content inside "Grants, Subawards & Amendments" subheader on left panel
    And I click toggle button to select "Grants - Active"
    When I perform quick search for "{SavedValue:Automation Runtime Formula Grant for Closeout}" in "---subAwardStandAlone:-:activeGrantsTableId---" panel
    And I click on "View" icon for "{SavedValue:Automation Runtime Formula Grant for Closeout}" inside table
    And I save the field labeled "EGMS ID" as "GrantEGMSID"
    When I navigate to "Subawards" sub tab
    And I click on "View" icon for "{SavedValue:Automation Runtime Formula Grant for Closeout}" inside flex table with id "---subAwardStandAlone:-:subawardsTableId---" without waiting for record
    And I save the field labeled "EGMS ID" as "AwardEGMSID"
    Then I softly see that "Subaward" is in "Activated" status
    # Navigate back to grant to verify status
    And I navigate to "Grants" tab
    When I navigate to "Grants" content inside "Grants, Subawards & Amendments" subheader on left panel
    And I click toggle button to select "Grants - Active"
    When I perform quick search for "{SavedValue:Automation Runtime Formula Grant for Closeout}" in "---subAwardStandAlone:-:activeGrantsTableId---" panel
    ...
    # ── Create ad-hoc closeout request and approve it ──
    When I navigate to "Requests" sub tab
    And I click on top right button "New" in flex table with id "---closeout:-:closeoutTableId---"
    When I enter in modal "Creation" values from "Closeout_Field_Values.xlsx"
    And I click modal button "Save and Continue"
    And I enter "Edition" values from "Closeout_Field_Values.xlsx"
    And I enter value "Automation Test justification for formula grant" into field "Justification__c"
    When I click on "Save" in the page details
    And I get the "EGMS ID"
    And I save the field labeled "EGMS ID" as "closeoutID"
    When I navigate to "Responsibilities" sub tab
    ...
    When I click on "Submit for Approval" in the page details
    And I wait for "2" seconds
    # Approver (EXE) approves the closeout, moving the grant to Closed/Completed
    When I navigate to "Closeout" tab
    And I navigate to "Pending Tasks" content inside "My Tasks" subheader on left panel
    When I perform quick search for "{SavedValue:closeoutID}" in "---closeout:-:pendingTaskTableId---" panel
    When I click on "Start" icon for "{SavedValue:closeoutID}" inside flex table with id "---closeout:-:pendingTaskTableId---"
    ...
    When I navigate to "Grants" content inside "Grants, Subawards & Amendments" subheader on left panel
    And I click toggle button to select "Grants - All"
    When I perform quick search for "{SavedValue:Automation Runtime Formula Grant for Closeout}" in "---closeout:-:grantsTableId---" panel
    And I click on "View" icon for "{SavedValue:Automation Runtime Formula Grant for Closeout}" inside flex table with id "---closeout:-:grantsTableId---"
    And I wait for "5" seconds
    Then I softly see field "Status" as "Closed/Completed"

    # ── Verification: EXE user sees Cumulative Obligation and Total Commitment on Budget tab ──
    When I navigate to "Budget" sub tab
    Then I softly see "Budget Summary by Focus Area" page block displayed
    Then I softly see field "Cumulative Obligation" as "$1,000.00"
    Then I softly see field "Total Commitment" as "$1,000.00"