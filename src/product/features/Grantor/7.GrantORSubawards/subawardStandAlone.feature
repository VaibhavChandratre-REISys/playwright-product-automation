...
#    When I close "Upload Document" modal by clicking the top right x button
  ...
  @530977 @validatecompetitiveGrant/awardrecordshouldsharewithPO/FO/FD/EXE/Auditor/AdminfromsameOrguserin(createdstateofaward) @GrantorFlexP1
  Scenario Outline: Validate Competitive Grant/award record should share with PO/FO/FD/EXE/Auditor/Admin from same Org user in (created state of award)
    ...
    And I softly see field "EGMS ID" as "{SavedValue:GrantEGMSID}"
    And I navigate to "Files" sub tab
    Then I see "Automation" inside flex table with id "---subAwardStandAlone:-:flexGridGrantChecklistsTableId---"
    And I navigate to "Subawards" sub tab
    And I click on "View" icon for "Created" inside flex table with id "---subAwardStandAlone:-:subawardsTableId---" without waiting for record
    ...
    Then I see value "{SavedValue:PO Username}" for title "Reviewer Name" inside table "---subAwardStandAlone:-:peerReviewersTableId---"
    Then I cannot see top right button "New" in flex table with id "---subAwardStandAlone:-:awardApproversTableId---"
    Then I see value "{SavedValue:PM Username}" for title "Name" inside table "---subAwardStandAlone:-:awardApproversTableId---"
    Then I see value "Automation FO" for title "Name" inside table "---subAwardStandAlone:-:awardApproversTableId---"
    And I navigate to "Files" sub tab
    Then I softly see value "Attachment.pdf" for title "Title" inside table "---subAwardStandAlone:-:addFilesTableId---"
    ...
    Then I softly can see row level action button "Download" against "Attachment.pdf" in flex table with id "---subAwardStandAlone:-:addFilesTableId---"
    Then I <Validation> top right button "New" in flex table with id "---subAwardStandAlone:-:notesAwardTableId---"
    Then I softly see value "Notes Record" for title "Title" inside table "---subAwardStandAlone:-:notesAwardTableId---"
    Examples:
      | NonOwner | Validation |
      | EXE      | can see    |
  ...
  @530986 @530985 @533441 @530983 @validateSPI/SPAexternalusershouldseegrantandawardrecordoncegrantorsendthatrecordtoS/Ruser @GrantorFlexP1
  Scenario Outline: Validate SPI/SPA external user should see grant and award record once grantor send that record to S/R user
  /Validate PM user can not share the created grant/award record ID to SPI/SPA user through chatter
    And I save the field labeled "EGMS ID" as "AwardEGMSID"
    And I navigate to "Overview" sub tab
    And I save the field labeled "Subaward Title" as "Automation Runtime award"
    And I click on "Edit" in the page details
    ...
    And I post the message "@{SavedValue:SPI3 Username} Automation Test1 Execution" inside chatter
    And I post the message "@{SavedValue:PO Username} Automation Test1 Execution" inside chatter
    #530986
    ...
    And I wait for "2" seconds
    Then I softly do not see notification "Automation Test1 Execution"
    And I navigate to "Grants" tab
    When I navigate to "Grants" content inside "Grants, Subawards & Amendments" subheader on left panel
    And I click toggle button to select "Grants - All"
    And I wait for "2" seconds
    When I perform quick search for "{SavedValue:GrantEGMSID}" in "---amendment:-:recipientActiveGrantsTableId---" panel
    Then I softly see "No Records Found" inside flex table with id "---amendment:-:recipientActiveGrantsTableId---"
    When I navigate to "Subawards" content inside "Grants, Subawards & Amendments" subheader on left panel
    And I click toggle button to select "Subawards  - All"
