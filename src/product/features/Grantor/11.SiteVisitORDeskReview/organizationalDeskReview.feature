@deskReview @organizationalDeskReview @grantor-regression @grantor-parallel-regression @siteVisit @regression @siteAndDeskReview @deskExecution
Feature: Validate all scenarios in Organizational Desk Review
  ...
  Scenario: Validate user view the Submit for Approval button when record sent back to Recipients when outcomes are not properly addressed and recipients will be able to receive a pending task
    ...
    Given I navigate to "Related Log" sub tab
    When I perform quick search for "Record Id" in "---orgLevelDeskReview:-:deskreviewPanelId---" panel
    Then I softly see that "Record Id" has been added in flextable with Id "---orgLevelDeskReview:-:deskreviewPanelId---"
    Then I softly see value "In Progress" for title "Status" inside table "---orgLevelDeskReview:-:deskreviewPanelId---"
  ...
  @55797 @organizationalDeskReviewPurpose  @organizationalDeskReviewFirst @ErrorMsgTest
  ...
  @55798 @organizationalDeskReviewSource  @organizationalDeskReviewFirst @ErrorMsgTest
  ...
  Scenario: User will be able to review the supporting document checklist submitted by the subrecipient
    ...
    Then I softly see value "View" for title "Template Link" inside table "---orgRiskAssessment:-:orgriskSupportingDocTableId---"
...
#    And I click modal button "Close"
  ...
  Scenario: Validate the other than record onwer FO user cannot delete the created Org Level Desk Review
    Given I navigate to "Related Log" sub tab
    And I click on top right button "Initiate Desk Review" in flex table with id "DeskReview"
    And I enter "Creation" values from "Organizational_DeskReview_Field_Values.xlsx"
    When I click modal button "Save and Continue"
    ...
    Then I cannot see row level action button "Delete" against "Record Id" in flex table with id "DeskReview"
