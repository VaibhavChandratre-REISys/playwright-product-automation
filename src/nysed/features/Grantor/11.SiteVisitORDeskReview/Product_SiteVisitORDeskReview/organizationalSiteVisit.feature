@organizationalSiteVisit @grantor-regression @grantor-parallel-regression @siteVisit @regression @siteVisit_Lwc_Run @siteAndDeskReview
...
#    When I click "Add" after selection of "{SavedValue:Automation Runtime Announcement for Site Visit}" in the table "---orgLevelSiteVisits:-:selectApplicationId---"
  ...
  Scenario: Filling up of form in Site Visit Org Level
    ...
    When I navigate to "Overview" sub tab
    And I click on top right button "Associate" in flex table with id "---orgLevelSiteVisits:-:associateContactsId---"
    And I perform quick search for "{SavedValue:SPI1 Username}" in "---orgLevelSiteVisits:-:siteVisitContactsTableId---" panel
    When I click "Associate" after selection of "{SavedValue:SPI1 Username}" in the table "---orgLevelSiteVisits:-:siteVisitContactsTableId---"
    And I pause execution for "3" seconds
    And I click modal button "Close"
    When I get the "EGMS ID"
...
#    When I close "Site Visit Reviewers" modal by clicking the top right x button
  ...
  Scenario: Validate whether the PM user can add the attachments to Org Level Site Visits and view the Created Date on Overview Tab |Validate whether record owner has access to delete the asoociated Contact and Application- Org Site visits
    ...
    And I click on top right button "Associate" in flex table with id "---orgLevelSiteVisits:-:associateContactsId---"
    When I click "Associate" after selection of "{SavedValue:SPI1 Username}" by showing entries "All" in flex table with id "---orgLevelSiteVisits:-:siteVisitContactsTableId---"
    And I click on "Remove" icon for "{SavedValue:SPI1 Username}" inside flex table with id "---orgLevelSiteVisits:-:associateContactsId---"
