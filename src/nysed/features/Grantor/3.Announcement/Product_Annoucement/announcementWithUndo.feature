@regression @announcement-undo  @Product-M04 @announcementExecution @Productannouncement
Feature: Validate all scenarios in the Announcement with Undo

  @283963 @283966 @283973 @283965 @VerifywhenPmapprovercanclickonundobuttoninAnnouncementrecordwhenrecordisinApprovedstate @sprint-4-US-295141  @Undo @Bug-Ticket-NYSED-3271
...
And I enter value "{SavedValue:Automation Runtime Active Program}" into field "Program__c"
    ...
    #283965
    And I checkout to yopmail with mailid "{SavedValue:PM1YopmailId}"
   ...
   The EGMS Team
    """
    And I checkout to yopmail with mailid "{SavedValue:EXEYopmailId}"
    And I pause execution for "10" seconds
    Then I refresh the page
    And I see the mail subject is as follows :
      """
  ...
  """
   ...
   The EGMS Team
  """
    And I checkout to yopmail with mailid "{SavedValue:POYopmailId}"
    And I pause execution for "10" seconds
    Then I refresh the page
    And I see the mail subject is as follows :
      """
  ...
  """
   ...
   The EGMS Team
  """
    And I checkout to yopmail with mailid "{SavedValue:PMYopmailId}"
...
And I enter value "{SavedValue:Automation Runtime Active Program}" into field "Program__c"
    ...
    Then I softly see value "Cancelled" for title "Status" against the value "{SavedValue:Automation Runtime Announcement}" inside table "---announcement:-:announcementCompletedTaskTable---"
    When I re-login to "As a Grantor" app as "EXE" user on "INTERNAL" portal
    And I navigate to "Announcements" tab
    And I navigate to "Completed Tasks" content inside "My Tasks" subheader on left panel
    When I perform quick search for "Record Id" in "---announcement:-:announcementCompletedTaskTable---" panel
    Then I softly see value "Completed" for title "Status" against the value "{SavedValue:Automation Runtime Announcement}" inside table "---announcement:-:announcementCompletedTaskTable---"
    When I re-login to "As a Grantor" app as "PO" user on "INTERNAL" portal
    And I navigate to "Announcements" tab
    And I navigate to "Completed Tasks" content inside "My Tasks" subheader on left panel
    When I perform quick search for "Record Id" in "---announcement:-:announcementCompletedTaskTable---" panel
...
And I enter value "{SavedValue:Automation Runtime Active Program}" into field "Program__c"
  ...
  Scenario: Verify that User Will Not See Undo when Announcement is in Closed state
    ...
    When I navigate to "Competitive" content inside "Announcements" subheader on left panel
    And I click toggle button to select "Competitive Announcements - All"
    And I perform quick search for "{AUTOEnvData:ClosedCompetitiveAnnouncement}" in "---announcement:-:competitiveAnnouncementTableId---" panel
    When I click on "View" icon for "{AUTOEnvData:ClosedCompetitiveAnnouncement}" inside flex table with id "---announcement:-:competitiveAnnouncementTableId---" without waiting for record
