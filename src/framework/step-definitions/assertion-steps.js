 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import { Then, When } from '@cucumber/cucumber';
...
async function softAssert(usage, fn) {
  if (usage.startsWith('softly')) {
    try { await fn(); } catch (e) { logger.warn(`[SOFT ASSERT] ${_optionalChain([e, 'access', _ => _.message, 'optionalAccess', _2 => _2.substring, 'call', _3 => _3(0, 200)])}`); }
  } else {
    await fn();
  }
}
...
Then(
  ...
  async function ( _usage, record, status) {
    const ft = new FlexTablePage(this.page, this.savedValues);
...
Then(
  ...
  async function ( _usage, text, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
...
Then(
  ...
  async function ( usage, columnTitle, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
...
Then(
  ...
  async function ( _usage, status, color) {
...
Then(
  ...
  async function ( usage, _matchExact, fieldName, fieldValue) {
    const pd = new PageDetailsPage(this.page, this.savedValues);
...
Then(
  ...
  async function ( _usage, fieldName, fieldValue) {
    const pd = new PageDetailsPage(this.page, this.savedValues);
    await pd.assertFieldValue(fieldName, fieldValue);
  }
);
...
Then(
  ...
  async function ( _usage, isContains, text) {
    ...
    if (_usage.startsWith('softly')) {
      try { await expect(locator).toBeVisible({ timeout: 15000 }); } catch (e3) { /* soft */ }
    } else {
      await expect(locator).toBeVisible({ timeout: 15000 });
    }
  }
...
Then(
  ...
  async function ( usage, isContains, dataTable) {
    const messages = dataTable.raw().flat().filter((s) => s.trim());
...
Then(
  ...
  async function ( usage, containsParam, dataTable) {
    const messages = dataTable.raw().flat().filter((s) => s.trim());
...
Then(
  ...
  async function ( usage, _contains, dataTable) {
    const messages = dataTable.raw().flat().filter((s) => s.trim());
    for (const msg of messages) {
      const resolved = this.savedValues.resolve(msg);
...
When(
  ...
  async function ( usage, buttonName) {
    const shouldExist = usage.includes('can see') && !usage.includes('cannot');
    const pd = new PageDetailsPage(this.page, this.savedValues);
...
When(
  ...
  async function ( usage, buttonName, tableId) {
    const shouldExist = usage.includes('can see') && !usage.includes('cannot');
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
...
Then(
  ...
  async function ( usage, subTabName) {
    const shouldExist = usage.includes('can see') && !usage.includes('cannot');
    const pd = new PageDetailsPage(this.page, this.savedValues);
...
Then(
  ...
  async function ( usage, fieldLabel) {
...
Then(
  ...
  async function ( usage, value1, value2) {
...
Then(
  ...
  async function ( usage, columnValue, _columnTitle, tableId) {
    const shouldSee = usage.includes('see') && !usage.includes('do not');
    const ft = new FlexTablePage(this.page, this.savedValues);
    await softAssert(usage, async () => {
      if (shouldSee) {
...
Then(
  ...
  async function ( usage, columnValue, _columnTitle, _uniqueValue, tableId) {
    const shouldSee = usage.includes('see') && !usage.includes('do not');
    const ft = new FlexTablePage(this.page, this.savedValues);
    await softAssert(usage, async () => {
      if (shouldSee) {
...
Then(
  ...
  async function ( usage, tableValue, tableId) {
    const shouldSee = usage.includes('see') && !usage.includes('do not');
    const ft = new FlexTablePage(this.page, this.savedValues);
    await softAssert(usage, async () => {
      if (shouldSee) {
...
Then(
  ...
  async function ( headerType, headerText) {
    const pd = new PageDetailsPage(this.page, this.savedValues);
...
Then(
  ...
  async function ( dataTable) {
    const panels = dataTable.raw().flat().filter((s) => s.trim());
...
Then(
  ...
  async function ( dataTable) {
    const buttons = dataTable.raw().flat().filter((s) => s.trim());
...
Then(
  ...
  async function ( tableId, dataTable) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
    const headers = dataTable.raw().flat().filter((s) => s.trim());
...
When(
  ...
  async function ( _labelType, fieldName, _beOrContain, fieldValue) {
...
Then(
  ...
  async function ( _labelType, fieldLabel, key) {
    const pd = new PageDetailsPage(this.page, this.savedValues);
...
Then(
  ...
  async function ( labeledOrContaining, fieldLabel, fieldKey) {
    const pd = new PageDetailsPage(this.page, this.savedValues);
...
Then(
  ...
  async function ( usage, matchExact, fieldName, fieldValue) {
    const pd = new PageDetailsPage(this.page, this.savedValues);
...
Then(
  ...
  async function ( usage, buttonName, uniqueValue, tableId) {
    const shouldExist = usage.includes('can see') && !usage.includes('cannot');
    const ft = new FlexTablePage(this.page, this.savedValues);
    await ft.assertRowLevelButton(tableId, uniqueValue, buttonName, shouldExist);
  }
...
Then(
  ...
  async function ( _usage, fieldApiName, values) {
...
Then(
  ...
  async function ( _usage, tableId, _tableName) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
...
Then(
  ...
  async function ( _usage, _header, recordsName, _action) {
...
Then(
  ...
  async function ( usage, value, _tableId) {
...
Then(
  ...
  async function ( usage, footerLink) {
...
Then(
  ...
  async function ( usage, fieldName) {
    const resolved = this.savedValues.resolve(fieldName);
...
Then(
  ...
  async function ( fieldApiName, dataTable) {
    const resolved = this.savedValues.resolve(fieldApiName);
...
Then(
  ...
  async function ( text, _type, fieldName) {
    const resolved = this.savedValues.resolve(text);
...
Then(
  ...
  async function ( usage, blockName) {
...
Then(
  ...
  async function ( usage, blockName, dataTable) {
    const resolved = this.savedValues.resolve(blockName);
    const fields = dataTable.raw().flat().filter((s) => s.trim());
...
Then(
  ...
  async function ( _usage, fieldName) {
    const resolved = this.savedValues.resolve(fieldName);
...
Then(
  ...
  async function ( _usage, fieldApiName) {
    const resolved = this.savedValues.resolve(fieldApiName);
    const input = this.page.locator(
      `//c-fieldlwc[@data-fieldapiname='${resolved}']//input | //div[@data-field_api_name='${resolved}']//input`
    ).first();
    try { await expect(input).toBeHidden({ timeout: 5000 }); } catch (e12) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, fieldApiName) {
    ...
    try { await expect(input).toBeHidden({ timeout: 5000 }); } catch (e13) { /* soft */ }
  }
);

Then(
  /^I (softly see|see) field ID "([^"]*)" as "([^"]*)" in edit mode$/,
  async function ( _usage, fieldId, expectedValue) {
    ...
    try { await expect(input).toHaveValue(resolvedValue, { timeout: 10000 }); } catch (e14) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, fieldApiName) {
    ...
    try { await expect(checkbox).toBeVisible({ timeout: 10000 }); } catch (e15) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, fieldApiName) {
    ...
    try { await expect(textarea).toBeVisible({ timeout: 10000 }); } catch (e16) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, fieldLabel) {
    const resolved = this.savedValues.resolve(fieldLabel);
    const editInput = this.page.locator(
      `//dt//*[contains(text(),'${resolved}')]//ancestor::dt//following-sibling::dd//input[not(@disabled)]`
    ).first();
    try { await expect(editInput).toBeHidden({ timeout: 5000 }); } catch (e19) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( usage, field1, field2) {
...
Then(
  ...
  async function ( usage, fieldName, sectionName) {
    const resolvedField = this.savedValues.resolve(fieldName);
...
Then(
  ...
  async function ( usage, modalTitle) {
...
Then(
  ...
  async function ( _usage, buttonName) {
    const resolved = this.savedValues.resolve(buttonName);
    const btn = this.page.locator(
      `//div[contains(@class,'modal')]//button[text()='${resolved}' or @title='${resolved}']`
    ).first();
    try { await expect(btn).toBeVisible({ timeout: 10000 }); } catch (e24) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( usage, toggleName, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
...
Then(
  ...
  async function ( _usage, buttonName, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
    const resolved = this.savedValues.resolve(buttonName);
    const btn = this.page.locator(
    ...
    try { await expect(btn).toBeHidden({ timeout: 5000 }); } catch (e27) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( usage, value, fieldApiName) {
...
Then(
  ...
  async function ( _usage, value, fieldLabel) {
    const resolvedValue = this.savedValues.resolve(value);
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e31) { /* soft */ }
  }
);

Then(
  /^I (softly see|see) value "([^"]*)" inside dropdown field "([^"]*)" displayed$/,
  async function ( _usage, value, fieldApiName) {
    const resolvedValue = this.savedValues.resolve(value);
    const resolvedField = this.savedValues.resolve(fieldApiName);
    const locator = this.page.locator(
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e32) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, fieldLabel, expectedValue, pageBlock) {
    const resolvedField = this.savedValues.resolve(fieldLabel);
    const resolvedValue = this.savedValues.resolve(expectedValue);
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e33) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, fieldLabel) {
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e34) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( fieldLabel, _dateRef) {
    const resolved = this.savedValues.resolve(fieldLabel);
...
Then(
  ...
  async function ( _usage, bodyText) {
    const resolved = this.savedValues.resolve(bodyText);
    const locator = this.page.locator(
      `//div[contains(@class,'modal')]//*[contains(text(),'${resolved}')]`
    ).first();
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e35) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, buttonName) {
    ...
    try { await expect(btn).toBeVisible({ timeout: 10000 }); } catch (e36) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, helpText) {
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e37) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( usage, fieldName, markText) {
    const resolved = this.savedValues.resolve(fieldName);
...
Then(
  ...
  async function ( usage, content, subheader) {
...
Then(
  ...
  async function ( _usage, buttonName, tableId) {
    ...
    try { await expect(btn).toBeVisible({ timeout: 10000 }); } catch (e42) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( usage, value, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
    const resolved = this.savedValues.resolve(value);
    const locator = this.page.locator(
...
Then(
  ...
  async function ( _usage, value, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
    const resolved = this.savedValues.resolve(value);
    const locator = this.page.locator(
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e44) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, count, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e45) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, expectedValue, columnName, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e46) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, total, _columnName, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e47) { /* soft */ }
  }
);

Then(
  /^I (softly see|softly do not see) addition of two fields "([^"]*)" and "([^"]*)" is equal to "([^"]*)"$/,
  async function ( _usage, _field1, _field2, _result) {
...
Then(
  ...
  async function ( _usage, _totalField, _dataTable) {
...
Then(
  ...
  async function ( _usage, _resultField, _sourceField, _dataTable) {
...
Then(
  ...
  async function ( _usage, val1, comparison, val2) {
...
Then(
  ...
  async function ( usage, key1, key2) {
...
Then(
  ...
  async function ( usage, symbol, againstValue, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
...
Then(
  ...
  async function ( _usage, dataTable) {
...
Then(
  ...
  async function ( _usage, count, tableId) {
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e50) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage) {
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e52) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, phaseName, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
...
Then(
  ...
  async function ( _usage, recordId) {
    ...
    try { await expect(fileLocator).toBeVisible({ timeout: 10000 }); } catch (e53) { /* soft */ }
...
Then(
  ...
  async function ( usage, userName) {
    ...
    if (shouldSee) {
      try { await expect(userLocator).toBeVisible({ timeout: 10000 }); } catch (e54) { /* soft */ }
    } else {
      try { await expect(userLocator).not.toBeVisible({ timeout: 10000 }); } catch (e55) { /* soft */ }
    }
...
Then(
  ...
  async function ( usage, tableTitle) {
...
Then(
  ...
  async function ( _usage, count, moduleName) {
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e57) { /* soft */ }
  }
);

Then(
  /^I (softly see|see) recent "([^"]*)" records in global search box$/,
  async function ( _usage, _count) {
...
Then(
  ...
  async function ( value, _fieldName) {
    const resolved = this.savedValues.resolve(value);
    const locator = this.page.locator(
...
Then(
  ...
  async function ( _usage, filterName) {
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e60) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage) {
    ...
    try { expect(pages.length).toBeGreaterThan(1); } catch (e61) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, title) {
    ...
    try { expect(pageTitle).toContain(resolved); } catch (e62) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, optionValue, dropdownName) {
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e63) { /* soft */ }
  }
);

Then(
  /^I (softly see|see) dropdown option "([^"]*)" under the "([^"]*)" section$/,
  async function ( _usage, optionValue, sectionName) {
    const resolvedOption = this.savedValues.resolve(optionValue);
    const resolvedSection = this.savedValues.resolve(sectionName);
    const locator = this.page.locator(
    ...
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e64) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, fieldName, dataTable) {
    const resolved = this.savedValues.resolve(fieldName);
    const options = dataTable.raw().flat().filter((s) => s.trim());
    for (const opt of options) {
      const locator = this.page.locator(
      ...
      try { await expect(locator).toBeVisible({ timeout: 5000 }); } catch (e65) { /* soft */ }
    }
  }
);
...
Then(
  ...
  async function ( usage, fieldName, value, _listName) {
    const resolvedField = this.savedValues.resolve(fieldName);
    const resolvedValue = this.savedValues.resolve(value);
    const locator = this.page.locator(
      `//div[@data-field_api_name='${resolvedField}']//*[contains(text(),'${resolvedValue}')]`
...
Then(
  ...
  async function ( _usage, _label, fieldApiName, expectedValue) {
    const resolvedField = this.savedValues.resolve(fieldApiName);
    const resolvedValue = this.savedValues.resolve(expectedValue);
    const locator = this.page.locator(
      `//c-fieldlwc[@data-fieldapiname='${resolvedField}']//*[contains(text(),'${resolvedValue}')]`
    ).first();
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e67) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, imageName, _expectedState) {
    const resolved = this.savedValues.resolve(imageName);
    const locator = this.page.locator(
      `//img[contains(@alt,'${resolved}') or contains(@title,'${resolved}')]`
    ).first();
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e68) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, name) {
    const resolved = this.savedValues.resolve(name);
    const locator = this.page.locator(`//*[contains(text(),'${resolved}')]`).first();
    try { await expect(locator).toBeVisible({ timeout: 10000 }); } catch (e69) { /* soft */ }
  }
);

Then(
  /^I (softly see|see) snapshot with user containing "([^"]*)" against the value "([^"]*)"$/,
  async function ( _usage, _userName, _value) {
...
Then(
  ...
  async function ( _usage, _colName, _position, _text) {
...
Then(
  ...
  async function ( _usage) {
    ...
    try { await expect(modal).toBeVisible({ timeout: 10000 }); } catch (e71) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( fieldName, tableId) {
    const ft = new FlexTablePage(this.page, this.savedValues);
    const resolvedId = ft.resolveTableId(tableId);
    const resolved = this.savedValues.resolve(fieldName);
...
Then(
  ...
  async function ( recordValue, fieldName) {
...
Then(
  ...
  async function ( dataTable) {
    ...
    for (const field of fields) {
      ...
      try { await expect(locator).toBeVisible({ timeout: 5000 }); } catch (e72) { /* soft */ }
    }
  }
);
    ...
    for (const row of rows) {
      for (const [header, value] of Object.entries(row)) {
        if (value) {
          ...
          try { await expect(locator).toBeVisible({ timeout: 5000 }); } catch (e73) { /* soft */ }
        }
      }
    }
  }
    ...
    for (const row of rows) {
      for (const [_header, value] of Object.entries(row)) {
        if (value) {
          ...
          try { await expect(locator).toBeVisible({ timeout: 5000 }); } catch (e74) { /* soft */ }
        }
      }
    }
  }
...
Then(
  ...
  async function ( usage, buttonName) {
    ...
    if (usage.includes('not')) {
      try { await expect(btn).toBeHidden({ timeout: 5000 }); } catch (e75) { /* ok */ }
    } else {
      await expect(btn).toBeVisible({ timeout: 10000 });
    }
  }
...
Then(
  ...
  async function ( dataTable) {
    ...
    for (const msg of messages) {
      const resolved = this.savedValues.resolve(msg);
      const locator = this.page.locator(`//*[contains(text(),'${resolved}')]`).first();
      try { await expect(locator).toBeHidden({ timeout: 5000 }); } catch (e76) { /* soft */ }
    }
  }
);
...
Then(
  ...
  async function ( _usage, fieldApiName, _value) {
    const resolved = this.savedValues.resolve(fieldApiName);
    const input = this.page.locator(
      `//c-fieldlwc[@data-fieldapiname='${resolved}']//input[not(@disabled)]`
    ).first();
    try { await expect(input).toBeHidden({ timeout: 5000 }); } catch (e77) { /* soft */ }
  }
);
...
Then(
  ...
  async function ( _usage, dataTable) {
    const roles = dataTable.raw().flat().filter((s) => s.trim());
    for (const role of roles) {
      const locator = this.page.locator(`//option[text()='${role}'] | //span[@title='${role}']`).first();
      try { await expect(locator).toBeVisible({ timeout: 5000 }); } catch (e78) { /* soft */ }
    }
  }
);
...
Then(
  ...
  async function ( usage, scheduleType, _periodType) {
...
Then(
  ...
  async function ( _usage, iniKey, decreasedByKey, currentKey) {
    try {
...
Then(
  ...
  async function ( usage, expectedValue, fieldLabel) {
    const shouldSee = usage.includes('see') && !usage.includes('do not');
