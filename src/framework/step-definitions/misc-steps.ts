...
When(
  ...
  async function (this: CustomWorld, tabIndex: string) {
...
When(
  ...
  async function (this: CustomWorld, buttonName: string, buttonLocation: string) {
    ...
    const tryDropdown = async (): Promise<boolean> => {
    ...
    if (!buttonLocation.includes('without processing') && !buttonLocation.includes('immediately')) {
      ...
      try {
        ...
        if (await continueButton.isVisible({ timeout: 2000 })) {
          ...
          logger.info(`  Confirmation popup detected (${buttonText?.trim()} button found), clicking...`);
          await continueButton.click();
          logger.info(`  ✓ Accepted confirmation popup via ${buttonText?.trim()} button`);
          popupHandled = true;
      ...
      if (!popupHandled) {
        try {
          ...
          if (await confirmationPopup.isVisible({ timeout: 2000 })) {
            ...
            logger.info(`  ✓ Accepted confirmation popup via ${buttonText?.trim()}`);
            popupHandled = true;
            await this.page.waitForTimeout(500);
          }
        } catch (e) {
...
When(
  ...
  async function (this: CustomWorld, fileName: string, fileType: string, _description: string) {
    const resolved = this.savedValues.resolve(fileName);
    const testDataDir = path.resolve(process.cwd(), 'src', 'project', 'test-data');
    const filePath = path.join(testDataDir, resolved);
...
When(
  ...
  async function (this: CustomWorld, fileName: string, _field: string, _fileType: string) {
    const resolved = this.savedValues.resolve(fileName);
    const testDataDir = path.resolve(process.cwd(), 'src', 'project', 'test-data');
    const filePath = path.join(testDataDir, resolved);
...
When(
  ...
  async function (this: CustomWorld, fileName: string, tableId: string) {
...
const deleteExcelHandler = async function (this: CustomWorld, fileName: string) {
  const resolved = this.savedValues.resolve(fileName);
  const testDataDir = path.resolve(process.cwd(), 'src', 'project', 'test-data');
  const filePath = path.join(testDataDir, resolved);
...
Then(
  ...
  async function (this: CustomWorld, colName: string, colValue: string, fileName: string) {
...
Then(
  ...
  async function (this: CustomWorld, row: string, value: string, colName: string, fileName: string) {
...
const checkoutYopmailHandler = async function (this: CustomWorld, mailportal: string, mailId: string) {
...
const navigateYopmailHandler = async function (this: CustomWorld, mailId: string, subject: string) {
...
When(
  ...
  { timeout: 300000 },
  async function (this: CustomWorld, mailId: string, subject: string) {
    const resolvedMail = this.savedValues.resolve(mailId);
    const resolvedSubject = this.savedValues.resolve(subject);
...
const seeMailSubjectHandler = async function (this: CustomWorld, docString: string) {
  ...
  try { await expect(mailSubject).toBeVisible({ timeout: 15000 }); } catch { /* soft */ }
};
...
const sendEmailCollabHandler = async function (this: CustomWorld, dataTable: any) {
...
Then(
  ...
  async function (this: CustomWorld, usage: string, fileName: string) {
    const resolved = this.savedValues.resolve(fileName);
...
Then(/^I (softly verify|verify) that "([^"]*)" file is downloaded$/, async function (this: CustomWorld, usage: string, fileName: string) {
  const resolved = this.savedValues.resolve(fileName);
...
Then(/^I (softly verify|verify) that the file "([^"]*)" is downloaded$/, async function (this: CustomWorld, usage: string, fileName: string) {
  const resolved = this.savedValues.resolve(fileName);
...
Then(
  ...
  async function (this: CustomWorld, usage: string, text: string, pageNum: string, pdfFile: string) {
...
Then(
  ...
  async function (this: CustomWorld, usage: string, pdfFile: string, dataTable: any) {
    const resolvedFile = this.savedValues.resolve(pdfFile);
...
Then(
  ...
  async function (this: CustomWorld, usage: string, value: string, title: string, _againstValue?: string) {
    const resolvedValue = this.savedValues.resolve(value);
