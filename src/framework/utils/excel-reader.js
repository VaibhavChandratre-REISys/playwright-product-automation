 ...
 */
export function readExcelFile(fileName, sheetName) {
  try {
    ...
    // Skip header row
    return data.slice(1).filter((row) => row.some((cell) => cell && cell.trim()));
  } catch (err) {
    if (_optionalChain([err, 'access', _ => _.message, 'optionalAccess', _2 => _2.includes, 'call', _3 => _3('Cannot find module')])) {
 ...
 */
export function deleteRowByColumnValue(columnName, columnValue, fileName) {
  ...
  for (let i = 1; i < data.length; i++) {
    const cellVal = String(_nullishCoalesce(data[i][columnIndex], () => ( ''))).trim();
 ...
 */
export function updateCellValue(value, row, column, fileName) {
  const filePath = resolveDownloadPath(fileName);
  if (!fs.existsSync(filePath)) {
    throw new Error(`File not found: ${filePath}`);
  }
...
export function assertCellValue(
  ...
  const actualValue = String(_nullishCoalesce(data[excelRowIndex][columnIndex], () => ( ''))).trim();
 ...
 */
export function verifyFileDownloaded(fileName, partialMatch = false) {
 ...
 */
export function verifyFileTypeDownloaded(fileType) {
  if (!fs.existsSync(DOWNLOADS_DIR)) {
    return false;
  }
