...
export class FailureDiagnostics {
   __init() {this.consoleLogs = []}
   static get DIAGNOSTICS_DIR() {
    const project = getProject();
    return `reports/${project}/diagnostics`;
  }

  constructor( page) {;this.page = page;FailureDiagnostics.prototype.__init.call(this);
      ...
      return await this.page.evaluate(() => {
        ...
        for (let i = 0; i < modals.length; i++) {
          ...
          if (rect.width > 0 && rect.height > 0) {
            ...
            if (header) {
              state.visibleModals.push(_optionalChain([header, 'access', _ => _.textContent, 'optionalAccess', _2 => _2.trim, 'call', _3 => _3()]) || 'Untitled Modal');
            }
          }
        }
        ...
        for (let i = 0; i < toasts.length; i++) {
          ...
          if (rect.width > 0 && rect.height > 0) {
            state.hasToast = true;
            state.toastMessage = _optionalChain([toasts, 'access', _4 => _4[i], 'access', _5 => _5.textContent, 'optionalAccess', _6 => _6.trim, 'call', _7 => _7()]) || '';
            break;
          }
        }
  ...
  /** Save all diagnostic artifacts to disk */
   async saveToDisk(
    ...
    const domContent = bundle.domSnapshot.length > 2000000
      ? bundle.domSnapshot.substring(0, 2000000) + '\n<!-- TRUNCATED -->'
      : bundle.domSnapshot;
   ...
   */
   async extractSmartDom() {
    try {
      return await this.page.evaluate(() => {
        const sections = [];
        const getHtml = (selector, label, maxLen = 5000) => {
   ...
   */
   async analyzeFailingLocator(errorMessage) {
    const lines = [
      '══ LOCATOR ANALYSIS ══',
      '',
    ];
        ...
        try {
          ...
          if (count > 0) {
            const firstHtml = await loc.first()
              .evaluate((el) => el.outerHTML.substring(0, 300))
      ...
      try {
        ...
        if (containsCount > 0 && containsCount <= 10) {
          for (let i = 0; i < Math.min(containsCount, 5); i++) {
            const info = await this.page.locator(`//*[contains(text(),'${searchText}')]`).nth(i).evaluate((el) => {
    ...
    if (tableNameMatch) {
      ...
      try {
        ...
        if (tableExists > 0) {
          ...
          const buttons = await this.page.evaluate((tn) => {
