...
export class SavedValues {
  ...
  hasCaseInsensitive(key: string): boolean {
    if (this.store.has(key)) {
      return true;
    }
    const lowerKey = key.toLowerCase();
  ...
  resolve(input: unknown): string {
    ...
    result = result.replace(/\{SavedValue:([^}]+)\}/g, (match, key: string) => {
    ...
    result = result.replace(/\{---([^:]+):-:([^}]+)---\}/g, (_match, fileName: string, key: string) => {
    ...
    result = result.replace(/\{grantor_tableId:([^}]+)\}/g, (_, key: string) => {
      return GRANTOR_TABLE_ID_MAP[key.trim()] ?? key.trim();
    });

    // 5. {grantee_tableId:KEY} — static map (backward compat)
    result = result.replace(/\{grantee_tableId:([^}]+)\}/g, (_, key: string) => {
      return GRANTEE_TABLE_ID_MAP[key.trim()] ?? key.trim();
    });

    // 6. {grantor_sidebar:KEY} — static map (backward compat)
    result = result.replace(/\{grantor_sidebar:([^}]+)\}/g, (_, key: string) => {
      return GRANTOR_SIDEBAR_MAP[key.trim()] ?? key.trim();
    });
    ...
    result = result.replace(/\{([^:}]+):([^}]+)\}/g, (match, fileName: string, key: string) => {
    ...
    result = result.replace(/\{Date:([^}]+)\}/g, (match, spec: string) => {
      try {
        return parseDateToken(spec.trim());
      } catch (e) {
        logger.warn(`Failed to resolve {Date:${spec}}: ${(e as Error).message}`);
        return match;
      }
    });
  ...
  getAllKeysStartingWith(prefix: string): string[] {
 ...
 */
export const GRANTOR_TABLE_ID_MAP: Record<string, string> = {
 ...
 */
export const GRANTEE_TABLE_ID_MAP: Record<string, string> = {
 ...
 */
export const GRANTOR_SIDEBAR_MAP: Record<string, string> = {
