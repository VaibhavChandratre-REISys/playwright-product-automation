...
type ModifierChar = 'n' | 's' | 'm' | 'H' | 'h' | 'D' | 'd' | 'M' | 'L' | 'Y' | 'y' | 'u';

const UNIT_MS: Partial<Record<ModifierChar, number>> = {
...
function applyModifier(date: Date, modifier: string): Date {
  ...
  const char = trimmed.substring(0, 1) as ModifierChar;
 ...
 */
function formatJavaPattern(date: Date, pattern: string): string {
  let result = '';
  let i = 0;
 ...
 */
export function parseDateToken(dateFormatter: string): string {
