...
export class FlexTablePage extends BasePage {
   ...
   */
  resolveTableId(rawTableId: string): string {
    ...
    if (token) {
      ...
      return TABLE_ID_MAP[`${module}:${key}`] ?? TABLE_ID_MAP[key] ?? rawTableId;
    }
    return rawTableId;
  }
   ...
   */
  async clickTableActionIconNoId(iconName: string, uniqueValue: string): Promise<void> {
   ...
   */
  async assertHighlightPanelStatus(record: string, status: string): Promise<void> {
  ...
  async assertTextInFlexTable(tableId: string, text: string): Promise<void> {
    const resolvedId = this.resolveTableId(tableId);
  ...
  async assertTextNotInFlexTable(tableId: string, text: string): Promise<void> {
    const resolvedId = this.resolveTableId(tableId);
    const resolved = this.resolve(text);
    const locator = this.page.locator(
      `//c-lwc-flex-table[@data-flextablename='${resolvedId}']//*[normalize-space(text())='${resolved}']`
  ...
  topRightButtonLocator(tableId: string, buttonName: string): Locator {
    const resolvedId = this.resolveTableId(tableId);
    const xpathParts = [
   ...
   */
  async clickTopRightButton(tableId: string, buttonName: string): Promise<void> {
    const resolvedId = this.resolveTableId(tableId);
    ...
    } catch { /* button may not exist yet */ }
   ...
   */
  async constructColumnMapping(row: Locator, headerLabels: string[]): Promise<Record<string, number>> {
   ...
   */
  private async fillTableCell(cell: Locator, value: string): Promise<void> {
    ...
    if (await fieldLwc.count() > 0) {
      ...
      try {
        ...
        const result = await fieldLwc.evaluate((el: any, val: string) => {
          try {
            // Access shadow root
            ...
            const allElements = shadowRoot.querySelectorAll('*') as NodeListOf<Element>;
    ...
    if (checkboxElement) {
      ...
      if ((wantChecked && !isChecked) || (wantUnchecked && isChecked)) {
        try {
          ...
          await checkboxElement.evaluate((el: HTMLElement) => el.click());
          await this.page.waitForTimeout(300);
          return;
        } catch (e) {
          ...
          // Try force click
          try {
            await checkboxElement.evaluate((el: HTMLInputElement) => {
              if (el.type === 'checkbox') {
   ...
   */
  async performModalSpecificQuickSearchForLWC(tableId: string, searchTerm: string): Promise<void> {
    const resolvedId = this.resolveTableId(tableId);
    ...
    try {
      ...
      await searchInputBox.clear();
      await searchInputBox.evaluate((el: HTMLInputElement, val: string) => {
        el.value = val;
   ...
   */
  async selectCheckBoxes(flexTableId: string, ...uniqueColumnTexts: string[]): Promise<void> {
    ...
    if (uniqueColumnTexts.length === 0) {
      ...
      await checkAllSelector.evaluate((el: HTMLElement) => el.click());
    ...
    for (const uniqueColumn of uniqueColumnTexts) {
      ...
      await rowCheckboxSelector.evaluate((el: HTMLElement) => el.click());
   ...
   */
  async chooseRecordForAssociation(buttonName: string, flexTableId: string, ...recordUniqueValues: string[]): Promise<void> {
    const resolvedId = this.resolveTableId(flexTableId);
   ...
   */
  async clickAnyRightPannelButton(idOfPanelButton: string, buttonName: string): Promise<void> {
    ...
    await rightPanelButton.evaluate((el: HTMLElement) => el.click());
  }
   ...
   */
  async flexTableShowEntriesDropDownForLWC(flexTableId: string, numberOfEntries: string): Promise<void> {
    const resolvedId = this.resolveTableId(flexTableId);
    ...
    try {
      ...
      await entriesDropDown.evaluate((el: HTMLElement) => el.click());
      await elementDropDown.evaluate((el: HTMLElement) => el.click());
 ...
 */
export const TABLE_ID_MAP: Record<string, string> = {
