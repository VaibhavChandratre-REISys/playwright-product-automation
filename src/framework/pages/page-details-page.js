...
export class PageDetailsPage extends BasePage {
   __init() {this.idOfRecord = null}

  constructor(page, savedValues) {
    super(page, savedValues);PageDetailsPage.prototype.__init.call(this);;
  }
   ...
   */
  async clickTopRightButton(buttonName) {
   ...
   */
  async clickTopRightButtonWithoutProcessing(buttonName) {
    const resolved = this.resolve(buttonName);
   ...
   */
  async assertFieldValue(fieldLabel, expectedValue) {
    const resolved = this.resolve(fieldLabel);
   ...
   */
  async getFieldValueInForm(fieldLabel, matchLabelExactly) {
    const resolved = this.resolve(fieldLabel);
   ...
   */
  async assertFieldValueInForm(fieldLabel, matchesExactly, expectedValue, isSoftly) {
    ...
    try {
      expect(_optionalChain([foundValue, 'optionalAccess', _ => _.toLowerCase, 'call', _2 => _2()])).toBe(resolvedValue.toLowerCase());
   ...
   */
  async assertFieldNotEditable(fieldLabel) {
    const resolved = this.resolve(fieldLabel);
   ...
   */
  async assertFieldEditable(fieldLabel) {
    const resolved = this.resolve(fieldLabel);
    const input = this.page.locator(
      `//label[normalize-space(text())='${resolved}']/following-sibling::div//input | ` +
      `//label[normalize-space(text())='${resolved}']/following-sibling::div//textarea | ` +
   ...
   */
  async saveFieldValueAs(fieldLabel, key) {
   ...
   */
  async assertProgressBarStatus(status, color) {
   ...
   */
  async assertPageHeader(headerText) {
   ...
   */
  async assertPageSubHeader(headerText) {
    const resolved = this.resolve(headerText);
    const locator = this.page.locator(
      `//div[contains(@class,'page-header__name')]//span[contains(@title,'${resolved}') or contains(text(),'${resolved}')]`
    ).first();
   ...
   */
  async assertPageMessage(message, shouldContain) {
   ...
   */
  async assertTopRightButton(buttonName, shouldExist) {
    const resolved = this.resolve(buttonName);
    const btn = this.page.locator(this.topRightBtnXpath(resolved)).first();
    if (shouldExist) {
      ...
      if (!visible) {
        ...
        } catch (e4) { /* dropdown may not exist */ }
      }
   ...
   */
  async assertPageBlock(blockName, shouldExist) {
   ...
   */
  async assertTextOnPage(text, shouldContain) {
   ...
   */
  async enterFieldValue(fieldApiName, fieldValue, isModal = false) {
   ...
   */
  async clearFieldValue(fieldApiName, isModal = false) {
    const resolvedField = this.resolve(fieldApiName);
   ...
   */
  async enterValuesFromExcel(fileName, sheetName, isModal = false) {
  ...
  async assertSubTab(subTabName, shouldExist) {
   ...
   */
   getFieldLocator(fieldApiName, isModal) {
   ...
   */
   async findEditableWrapper(locator, timeout) {
   ...
   */
   async handleFieldInput(wrapper, value) {
    ...
    switch (fieldType) {
      ...
      default: {
        ...
        if (!fieldApiName) {
          fieldApiName = await wrapper.evaluate((el) => {
            let current = el;
            while (current && current !== document.body) {
              if (current.getAttribute && current.getAttribute('data-field_api_name')) {
                return current.getAttribute('data-field_api_name');
        ...
        if (fieldApiName) {
          ...
          if (textareaCount > 0) {
            ...
            await firstTextarea.evaluate((el) => el.click()).catch(() => {});
   ...
   */
   async fillRichTextArea(richTextEditor, value, fieldApiName) {
    ...
    if (!fieldApiName) {
      ...
      if (!fieldApiName) {
        ...
        fieldApiName = await richTextEditor.evaluate((el) => {
          let current = el;
          while (current && current !== document.body) {
            if (current.getAttribute && current.getAttribute('data-field_api_name')) {
              return current.getAttribute('data-field_api_name');
    ...
    if (fieldApiName) {
      ...
      if (count > 0) {
        ...
        await firstTextarea.evaluate((el, val) => {
          el.value = val;
    ...
    if (await editable.count() > 0) {
      ...
      if (_optionalChain([actual, 'optionalAccess', _3 => _3.trim, 'call', _4 => _4()]) !== value.trim()) {
   ...
   */
   async fillInputWithRetry(input, value) {
    ...
    const readValue = async () => {
      if (isContentEditable) {
        return await _asyncNullishCoalesce((await input.textContent().catch(() => '')), async () => ( ''));
      }
   ...
   */
   areNumbersEqual(displayed, entered) {
   ...
   areDatesEqual(displayed, entered) {
    try {
   ...
   */
   async handleLookupInput(wrapper, value) {
   ...
   */
   async handlePicklistInput(wrapper, value) {
    // Check if wrapper itself is the dropdown button (fallback locator matched button[name='X'] directly)
    const wrapperTag = await wrapper.evaluate((el) => el.tagName.toLowerCase()).catch(() => '');
   ...
   */
   async handleMultiSelectInput(wrapper, value) {
   ...
   */
   async handleDateInput(input, value) {
    let dateValue = value;
   ...
   */
  async clickTopRightButtonInsideSplitScreen(buttonName) {
    const resolved = this.resolve(buttonName);
   ...
   */
  async clickTopRightButtonInsideSplitScreenWithoutProcessing(buttonName) {
    const resolved = this.resolve(buttonName);
   ...
   */
  async clickSubTabsInsideTableViewForSplitScreen(tabName) {
