 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }import * as fs from 'fs';
 ...
 */
function getProjectPropertiesDir() {
 ...
 */
export function loadProperties(fileName) {
 ...
 */
export function getProperty(fileName, key) {
  const properties = loadProperties(fileName);
  return _nullishCoalesce(properties[key], () => ( ''));
}
 ...
 */
export function getPropertyWithDefault(fileName, key, defaultValue) {
  const properties = loadProperties(fileName);
  return _nullishCoalesce(properties[key], () => ( defaultValue));
}
