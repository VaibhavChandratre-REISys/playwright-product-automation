 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import { logger } from '../utils/logger';
...
export class AnnouncementPublisher {
  ...
  ) {;this.getIdOf = getIdOf;this.getRecordTypeId = getRecordTypeId;this.dateWithOffset = dateWithOffset;this.getEnvPropertiesFile = getEnvPropertiesFile;}

  /**
   * Build default announcement payload
   */
  async buildDefaultPayload(annType, annName, programName) {
   ...
   */
  async applyFilter(payload, filter) {
    const ann = payload.Announcement[0];
    let normalizedFilter = filter;

    // Handle user-based filter prefix
    if (filter.toLowerCase().includes('user')) {
      const user = _optionalChain([filter, 'access', _ => _.split, 'call', _2 => _2('-'), 'access', _3 => _3[1], 'optionalAccess', _4 => _4.split, 'call', _5 => _5('user'), 'access', _6 => _6[0], 'optionalAccess', _7 => _7.trim, 'call', _8 => _8()]);
      if (user) {
    ...
    switch (normalizedFilter) {
      ...
      case 'BUDGETPERIODDATES':
        if (_optionalChain([payload, 'access', _9 => _9.BudgetPeriod, 'optionalAccess', _10 => _10[0]])) {
