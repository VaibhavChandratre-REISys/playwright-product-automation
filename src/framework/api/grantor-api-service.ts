...
export class GrantorApiService {
  ...
  constructor(savedValues: SavedValues) {
   ...
   */
  async getIdOf(objectName: string, recordName: string): Promise<string> {
    const url = `${this.instanceUrl}/services/apexrest/getRecordIds/v1`;
    const payload: Record<string, string[]> = { [objectName]: [recordName] };
   ...
   */
  async getRecordTypeId(objectName: string, recordTypeName: string): Promise<string> {
   ...
   */
  async getMultipleIds(payload: Record<string, string[]>): Promise<Record<string, string>> {
    const url = `${this.instanceUrl}/services/apexrest/getRecordIds/v1`;
    const response = await this.client.post(url, payload, { headers: this.authHeaders() });
  ...
  private dateWithOffset(offsetDays: number): string {
    const date = new Date();
   ...
   */
  private async buildProgramPayload(programName: string): Promise<any> {
   ...
   */
  async activateProgram(programNameKey: string, filter: string = 'default'): Promise<string> {
    if (!this.accessToken) {
      await this.authenticate();
    }
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      try {
        ...
        if (error.response && error.response.status === 500) {
          ...
          if (Array.isArray(errorBody) && errorBody[0]?.message?.includes('Fund Code')) {
   ...
   */
  async publishAnnouncement(annType: string, annNameKey: string, filter: string = 'default'): Promise<string> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  async createStandaloneGrant(announcementNameKey: string, filter: string = 'default'): Promise<string> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  async activateStandaloneSubaward(announcementNameKey: string, filter: string = 'default'): Promise<string> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  async approveFDM(applicationName: string, filter: string = 'default'): Promise<void> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  async createAwardFromFDM(announcementName: string, properties: string = 'default'): Promise<void> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  async activateAwardFromFDM(applicationName: string): Promise<void> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  private defaultPayloadApplicationReview(applicationName: string, announcementName: string, ids: Record<string, string>, recordTypeId: string): any {
    const ownerName = 'Automation SPI3';
    const contactName = 'Automation SPI3';
    const kpiName = 'Automation Permanent KPI';
   ...
   */
  private updatePlumbingCase(defaultJson: any, announcementName: string, applicationName: string, ids: Record<string, string>, recordTypeId: string): any {
    const ownerName = 'Automation SPI3';
    const contactName = 'Automation SPI3';
   ...
   */
  private updateBudgetCategoryForOneFocusAreasDetailsBudgetYes(defaultJson: any): any {
    const budgetCategoryArray: any[] = [];
   ...
   */
  private updateBudgetCategoryForTwoFocusAreasDetailsBudgetYes(defaultJson: any): any {
    const budgetCategoryArray: any[] = [];
   ...
   */
  private updateBudgetCategoryForFirstFocusAreasNoFocusArea(defaultJson: any): any {
    const budgetCategoryArray: any[] = [];
   ...
   */
  private updateBudgetCategoryForTwoFocusAreasFifteenHundredAmt(json: any): any {
    const budgetCategoryArray: any[] = [];
   ...
   */
  private updateBudgetCategoryNoFocusAreas(json: any): any {
    const budgetCategoryArray: any[] = [];
   ...
   */
  private updateBudgetCategoryForFirstFocusAreas(defaultJson: any): any {
    const budgetCategoryArray: any[] = [];
   ...
   */
  private updateBudgetCategoryForTwoFocusAreas(json: any): any {
    const budgetCategoryArray: any[] = [];
   ...
   */
  private updateAppOwnerAndContact(json: any, user: string, ids: Record<string, string>): void {
   ...
   */
  async completeReviewStep(module: string, applicationName: string): Promise<void> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  async createPostAwardRecord(objectType: string, recordName: string, objectApiName: string): Promise<string> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  async createRiskAssessment(organizationName: string): Promise<void> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  async deleteRecord(recordName: string, objectType: string): Promise<void> {
    if (!this.accessToken) {
      await this.authenticate();
    }
  ...
  async cleanupTestData(): Promise<void> {
    if (!this.accessToken) {
      try { await this.authenticate(); } catch { return; }
    }
    ...
    const safeDelete = async (recordName: string, objectType: string) => {
      try {
    ...
    const valueOf = (k: string) => this.savedValues.resolve(`{${k}}`);
   ...
   */
  private buildUpdateGrantAwardPayload(): any {
   ...
   */
  private async applyFDMFilter(payload: any, filter: string): Promise<void> {
   ...
   */
  private async applyApplicationReviewFilter(payload: any, filter: string): Promise<void> {
    const normalizedFilter = filter.toUpperCase();
    switch (normalizedFilter) {
  ...
  private async applySubawardFilter(payload: any, filter: string): Promise<void> {
    ...
    switch (normalizedFilter) {
      ...
      case 'CONTRACTSTOP_NULL':
        if (payload.Award?.[0]) payload.Award[0].projectStop = null;
        break;
      case 'PROJECTSTOP_PRESENT':
      case 'CONTRACTSTOP_PRESENT':
        if (payload.Award?.[0]) payload.Award[0].projectStop = this.dateWithOffset(150);
        break;
      case 'CONTRACTUAL_YES':
        if (payload.Award?.[0]) payload.Award[0].contractual = 'Yes';
        break;

      // ── Standalone Grant Award filters ──────────────────────────────────
      case 'NO_RISK_ASSESSMENT':
        if (payload.Grant?.[0]) payload.Grant[0].RiskAssessment_Required = 'No';
        break;
      case 'YES_RISK_ASSESSMENT':
        if (payload.Grant?.[0]) payload.Grant[0].RiskAssessment_Required = 'Yes';
        break;
      case 'NO_ADVANCE_PERMISSION':
        if (payload.Grant?.[0]) payload.Grant[0].IsAdvancePermitted = 'No';
        break;
      case 'IS_BUILDUP_FUNCTIONALITY_YES':
        if (payload.Grant?.[0]) payload.Grant[0].IsBuildUpFunctionality = 'Yes';
        break;
      case 'NO_PROGRAM_INCOME_ANTICIPATE':
        if (payload.Grant?.[0]) payload.Grant[0].IsProgramIncomeAnticipated = 'No';
        break;
      case 'NO_BUDGETREDIRECTIONTHRESHHOLD':
        if (payload.Grant?.[0]) payload.Grant[0].BudgetRedirectionThreshold = null;
        break;
      case 'AMENDMENT_REQUEST':
        if (payload.Grant?.[0]) payload.Grant[0].AmendmentRequest = 'Yes';
        break;
      ...
      case 'PROGRESS_REPORT_NOT_APPLICABLE':
        if (payload.TermConditionSchedule) {
          payload.TermConditionSchedule = payload.TermConditionSchedule.filter(
            (tc: any) => tc.SubmissionType !== 'Progress Report'
          );
        }
        break;
   ...
   */
  async activateSubaward(grantName: string, filter: string, type: string = 'Competitive'): Promise<void> {
    if (!this.accessToken) {
      await this.authenticate();
    }
   ...
   */
  private async buildDefaultStandaloneAwardPayload(grantName: string, type: string = 'Competitive'): Promise<any> {
    const envFile = this.getEnvPropertiesFile();
